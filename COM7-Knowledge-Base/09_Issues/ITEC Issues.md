...
## จากแบบสำรวจ data requirement (2026-08-28)

ผลสำรวจ 77 ฟิลด์ → [[ITEC Data Requirement Survey]]

### 🔴 ต้องหยุดก่อน

| เรื่อง | รายละเอียด | ใครตอบ |
|---|---|---|
| **คำขอเลขบัตรเครดิตเต็มใบ** | ผู้ตอบระบุ *"จริงๆอยากได้ทุกตัว"* — เลขบัตรเต็มอยู่ใต้ **PCI-DSS** ไม่ใช่แค่ PDPA<br>เสนอใช้ BIN (6 ตัวหน้า) + 4 ตัวท้าย + `APPROVAL CODE` แทน | **legal / DPO** |

### ฟิลด์ที่ตอบว่า "มี" แต่หาไม่เจอใน 23 view

| ฟิลด์ | อ้างถึง | ต้องถาม |
|---|---|---|
| product active status | ตาราง **`Product`** คอลัมน์ `EOLStatus` | ตารางนี้อยู่ไหน เข้าถึงยังไง |
| promotion code | ไม่ระบุ | รหัสโปรโมชันเก็บที่ไหน |
| branch manager name | ไม่ระบุ | ชื่อ ผจก. สาขาอยู่ตารางไหน |
| device activation date | เฉพาะ Apple | อยู่ตารางไหน |

> **แปลว่า ITEC มีข้อมูลมากกว่าที่ 23 view เปิดให้เห็น** — ต้องขอรายการตารางจริงหรือขอสิทธิ์เพิ่ม
> ไม่งั้นเราออกแบบ pipeline บนภาพที่ไม่ครบ

### ช่องว่างที่ไม่ใช่ความผิดของ ITEC

| หมวด | สถานะ | ควรไปถามที่ไหน |
|---|---|---|
| **after sale service** | 0 จาก 9 ฟิลด์ | ระบบ **iCare** (Timeline 1.1.6.2 · P.Pui) — ITEC มี iCare 61 สาขา ยอด 291 ล้านบาท/ปี แต่ไม่มีข้อมูลเคส |
| **logistic** | ไม่ตอบทั้ง 13 ฟิลด์ | **ต้องนิยาม scope ก่อน** — ผู้ตอบถามกลับว่า *"Logistic หมายถึงขนส่งของขั้นตอนไหน"* |
| **ข้อมูลพนักงาน** | 4 ฟิลด์ | **ฝ่ายบุคคล** — ผู้ตอบระบุเอง *"ต้องขอที่ฝ่ายบุคคล"* |
